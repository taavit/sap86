...
pub fn exec_adc(src: &Operand, dst: &Operand, cpu: &mut Cpu, machine: &mut Machine) {
    ...
    match dst {
        Operand::Register8(_) | Operand::Mem8(_) => {
            ...
            let (result, c2) = result.overflowing_add(if cpu.flags.carry { 1 } else { 0 });
            cpu.flags.zero = result == 0;
            cpu.flags.carry = c || c2;
